...
public class ByteUtils {
     ...
     */
    public static String fromBytes(byte[] data, String charsetName) throws UnsupportedEncodingException, UnsupportedEncodingException {
...
// transfer bytes from this buffer into the given destination array
    ...
    public static int ReverseHexToInt(String Hex) {
        byte[] AmountRecievedBytes = ByteUtils.hexString2ByteArray(Hex);
        AmountRecievedBytes = ByteUtils.reverse(AmountRecievedBytes);
        Hex = ByteUtils.byteArray2HexString(AmountRecievedBytes);
        return ByteUtils.hexToDecimal(Hex);
    }

    public static String IntToReverseHex(int Num) {
        String HexValue = ByteUtils.byteArray2HexString(
                ByteUtils.reverse(ByteUtils.intToBytes(Num)));
        return HexValue;
    }
    ...
    public static String XOR(String s1, String s2, int iLen) {
        byte[] hexs1 = ByteUtils.hexString2ByteArray(s1);
        byte[] hexs2 = ByteUtils.hexString2ByteArray(s2);

        return ByteUtils.byteArray2HexString(XORHEX(hexs1, hexs2, iLen));
    }
